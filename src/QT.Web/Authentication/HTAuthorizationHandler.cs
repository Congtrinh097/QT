...
namespace QT.Web.Authentication
{
    ...
    {
        private QTDBRepository QTDBRepository;
        public HTAuthorizationHandler(QTDBRepository _QTDBRepository)
        {
            this.QTDBRepository = _QTDBRepository;
        }
        ...
        {
            return QTDBRepository.ClaimRepository.GetAll().Any(x => x.ClaimName == claim);
        }
        private void CreateNewClaim(string claim)
        {
            ...
            QTDBRepository.ClaimRepository.Save(entity);
            QTDBRepository.Commit();
        }

        private bool CheckClaims(string requireClaim, int userId)
        {
            return QTDBRepository.UserRoleRepository.GetAll()
